...
export async function getPermissionsByRole(role: StaffRole): Promise<Permission[]> {
  ...
  console.log("Loaded permissions for role", role, ":", permissions.length);
...
export async function getAdminUser(): Promise<AdminUser | null> {
  ...
  console.log("Staff record found:", staff.role);

  // Get permissions based on role
  const permissions = await getPermissionsByRole(staff.role);

  console.log("Admin user loaded with", permissions.length, "permissions");
